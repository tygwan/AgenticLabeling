...
const ReviewWorkspace = ({ sources, settings, onUpdateObject, panelOrder }) => {
  const safeSources = (sources && sources.length) ? sources : [];
  const [activeId, setActiveId] = React.useState(safeSources[0]?.id || null);
  ...
  // When live data arrives or source identity changes, re-seed activeId.
  React.useEffect(() => {
    if (!activeId && safeSources[0]) setActiveId(safeSources[0].id);
    else if (activeId && !safeSources.some(s => s.id === activeId) && safeSources[0]) {
      setActiveId(safeSources[0].id);
    }
  }, [safeSources, activeId]);

  if (safeSources.length === 0) {
    return <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--c-muted)' }}>No sources yet. Upload from the Home screen to get started.</div>;
  }

  const source = safeSources.find(s => s.id === activeId) || safeSources[0];
  // Objects are read directly from the live source. Mutation goes through
  // onUpdateObject which App handles via optimistic API calls.
  const objects = source?.objects || [];
  const visibleObjs = objects;
  const current = visibleObjs.find(o => o.object_id === selectedObj) || null;

  // Filter sources
  const filteredSources = safeSources.filter(s => {
  ...
  const setValidated = (oid, v) => {
    if (onUpdateObject) onUpdateObject(source.id, oid, v);
  };
