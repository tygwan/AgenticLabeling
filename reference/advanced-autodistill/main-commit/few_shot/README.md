...
few_shot/
├── scripts/
│   ├── classifier_cosine.py          # 코사인 유사도 기반 분류기
│   ├── classifier_cosine_experiment.py  # 실험 자동화 모듈
│   ├── few_shot_webapp.py            # Gradio 기반 웹 인터페이스
│   └── debug_model.py                # 모델 디버깅 도구
├── run_few_shot_platform.py          # 플랫폼 메인 실행 스크립트
├── install_few_shot_deps.sh          # 의존성 설치 스크립트
└── few_shot_requirements.txt         # 필요한 패키지 목록
```
...
1. 모델 초기화 및 특징 추출 확인:
   ...
   python scripts/debug_model.py --model resnet --test-extraction
   ```
